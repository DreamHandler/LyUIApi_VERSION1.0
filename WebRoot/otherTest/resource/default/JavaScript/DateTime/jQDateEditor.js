includecss("/YxUI/Source/plugins/skins/Aqua/css/yxui-form.css");
/**
* jQuery YxUI 1.3.2
* 
* http://Yxui.com
...
(function ($)
{
    $.fn.YxDateEditor = function ()
    {
        return $.Yxui.run.call(this, "YxDateEditor", arguments);
    };

    $.fn.YxGetDateEditorManager = function ()
    {
        return $.Yxui.run.call(this, "YxGetDateEditorManager", arguments);
    };

    $.YxDefaults.DateEditor = {
    ...
    $.Yxui.controls.DateEditor = function (element, options)
    {
        $.Yxui.controls.DateEditor.base.constructor.call(this, element, options);
    };
    $.Yxui.controls.DateEditor.YxExtend($.Yxui.controls.Input, {
        __getType: function ()
        {
            return 'DateEditor';
        },
        ...
        _render: function ()
        {
           ...
           // g.link = $('<div class="l-trigger"><div class="l-trigger-icon"></div></div>');
            g.link = $('<span class="input-group-addon" style="cursor:pointer;"><i class="fa fa-calendar bigger-110"></i></span>');
            g.text = g.inputText.wrap('<div class="input-group"></div>').parent();
            //g.text.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
            g.text.append(g.link);
            //添加个包裹，
            g.textwrapper = g.text.wrap('<div style="width:150px;"></div>').parent();
            var dateeditorHTML = "";
            ...
            *************/
            ...
            {
                if (g.dateeditor.is(":visible") && $((e.target || e.srcElement)).closest( ".l-box-dateeditor, .input-group" ).length == 0)
                {
                    g.toggleDateEditor(true);
                }
            });
        ...
        toggleDateEditor: function (isHide)
        {
            var g = this, p = this.options;
            //避免同一界面弹出过个菜单的问题
            var managers = $.Yxui.find($.Yxui.controls.ComboBox);
            ...
            managers = $.Yxui.find($.Yxui.controls.DateEditor);
            for ( var i = 0, l = managers.length; i < l; i++) {
                var o = managers[i];
                if(o.id!=g.id){
