//数组或者XML的GRID
includecss("resource/default/css/TbBTGrid.css");
/**
 * 基与BootStrap属性的GRID
 * 此GRID不带任何编辑功能
 * 实现的属性有 columns ：
 ...
 */
var TbBTGrid = Class.create();
TbBTGrid.prototype = Object.extend(new TBase(),{
	ThisGrid : [],
	...
	createGrid : function(id,columns,options){
		...
		this.bindGrid = options.bindGrid == null ? "TbBTGrid" : options.bindGrid;
		...
		this.isSetColColor = options.isSetColColor == null ? false : options.isSetColColor;
		this.deccHeight = options.deccHeight;
		this.isIE = navigator.userAgent.indexOf('MSIE 7.0') > -1?true:false;
		this.choiceRow = []; //选择的行
		this.choiceCelll = []; //选择的单元格
		this.result = []; //数据集合
		this.cellObj = []; //单元格对象
		this.Aresult = null;
		...
		this.createDiv();
		this.addSpaceRow();
		TbBTGrid.prototype.ThisGrid[id] = this;
		//设置自动改变大小
//		this.element.css("overflow","hidden");
//		this.element.resize(function(){TbBTGrid.prototype.ThisGrid[id].autoResize()});
	},
	autoResize : function(){
//		if(!this.autoBool){
//			this.autoBool = true;
...
//		}
	...
	createColumnHeaders : function(){
		...
		var countWidth = 0;
		rowHtml.append("<table cellpadding=0 cellspacing=0 border=0 height='" + this.headerHeight + "px' style='margin-left:0px;'><tr>");
		this.resultTable = "<COLGROUP>";
		for(var i = 0;i < this.columns.length;i += 1){
			var column = this.columns[i];
			var colWidth = (column.width == null?50:parseInt(column.width)) ;
			var head_th_width = colWidth;
			if (column.status) {
				rowHtml.append("<td onclick='TbBTGridEvent.clickSortTh(this,\""+this.eleId+"\")' ><div class='container-fuild' align='center' id='span"+i+"' for='td"+i+"' style='width:" + colWidth + "px;cursor:pointer;' " +
						"onclick='TbBTGridEvent.changeOrder(this,"+i+","+column.status+",\""+this.eleId+"\")'>" 
						+ column.name+"&nbsp<span class='ace-icon fa fa-sort "+this.eleId+"_grid-sort'></span></div></td>");			
			}else{
				rowHtml.append("<td><div class='container-fuild' align='center' style='width:" + head_th_width + "px'>" + column.name+"</div></td>");
			};
		...
		this.resultTable = "<div style='width:" + (countWidth-1  ) + "px;margin-top:1px;'><table id='" + this.eleId + "_datas' cellpadding=0 cellspacing=0 border=0 class='table table-hover " + this.rowStyle + "' style='margin-left:0px;'>" + this.resultTable;
		this.headDiv.html("<div style='width:" + (countWidth) + "px;height:" + this.headerHeight + "px'>" + rowHtml.toString() + "</div>");
	},
	createDiv : function(){
		...
		this.resultDiv.scroll(function(){
			var maxWidth = _this.countWidth-_this.headDiv.width()-1;
	...
	showResult : function(datas) {
		...
		if(this.dataType == "xml" && datas.length == null){ //判断类型
			...
			this.Aresult = datas.selectSingleNode("Rows"); 
		...
		if (this.result.length != 0){
			...
			for (var i = 0; i < this.result.length; i += 1){			
				...
				for ( var j = 0; j < this.columns.length; j += 1){
					...
					data.append("<td style='border-right:1px solid #ddd;border-bottom:1px solid #ddd;padding:8px 0px'  name='"+column.name+"' fieldname='"+column.fieldname+"' onmousedown='" + this.bindGrid + ".prototype.dCellClick(this,event)' ><div class='container-fuild' align='center' style='width:" + column.width + "px'>" + (dataValue==null?"":dataValue) +"</div></td>");
				}				
				data.append("</tr>");
			}
	...
	addRow : function(data,Aclone){ //增加一行
		...
		if(data == null){
			...
			$row.mousedown(function(event){TbBTGrid.prototype.dRowClick($row[0],event)});
	...
	addSpaceRow : function(){
		var data = "<tr id='"+this.eleId+"_space_-1'><th/>";
		for(var i = 0;i < this.columns.length;i += 1){
			var column = this.columns[i];
			data += "<td>&nbsp;</td>";
		}
	...
	cell : function(row,col){ //获得指定单元格对象
		...
		if(this.cellObj[row][col] == null){
			this.cellObj[row][col] = new TbBTGridCell(this.rows(row).childNodes[col],this,col);
		}
	...
	dRowKeyDown : function(rowEle,aEvent){ //键盘事件
		...
		case 38:
			TbBTGridringGridEvent.isDisplay($(rowEle),"prev");
			if(inRow == null || inRow.attr("Ix") == null)
				return;
			break;
		case 40:
			inRow = TbBTGridEvent.isDisplay($(rowEle),"next");
			if(inRow == null || inRow.attr("Ix") == null)
				return;
			break;	
		default:
			var _this = TbBTGrid.prototype.ThisGrid[rowEle.getAttribute("grid")];
	...
	dCellClick : function(cellEle,aEvent){ //单元格单击事件
		var rowEle = $(cellEle).parent()[0];
		var _this = TbBTGrid.prototype.ThisGrid[rowEle.getAttribute("grid")];
		if(_this.enabled){
			if(_this.choiceCelll[0] != cellEle){ //设置单元格选
				if(_this.choiceCelll[0] != null){
					TbBTGridEvent.setColColor(_this, _this.choiceCelll[0], false);
					TbBTGridEvent.setRowColor(_this.choiceRow[0].childNodes,false);
				}
				TbBTGridEvent.setColColor(_this, cellEle, true); 
				TbBTGridEvent.setRowColor(rowEle.childNodes,true);
			}
			_this.choiceCelll[0] = cellEle;
			_this.choiceRow[0] = rowEle;
			//绑定事件
			TbBTGridEvent.bindCellAffair($(cellEle), _this, true, aEvent);
		}else{ //清空事件
			TbBTGridEvent.bindCellAffair($(cellEle), _this, false, aEvent);
		}
	},
	dRowClick : function(rowEle,aEvent){ //行单击事件
		...
		if(_this.enabled){
			...
			_this.choiceRow[0] = rowEle;
			//获取列属性
			TbBTGridEvent.bindAffair($(rowEle), _this, true, aEvent);
		}else{ //清空事件
			TbBTGridEvent.bindAffair($(rowEle), _this, false, aEvent);
		}
	},
	setRowSelect : function(InrowIndex){ //设置默认选中的行
		var rowIndex = (InrowIndex == null || InrowIndex == -2)?1:InrowIndex;
		var curRow = $(this.rows()[rowIndex]);
		curRow = curRow.css("display") == "none"?TbBTGridEvent.isDisplay(curRow,"next"):curRow;
		if(curRow.attr("Ix") != null ){
			if(this.choiceRow[0] != curRow[0]){
				if(this.choiceRow[0] != null)
					TbBTGridEvent.setRowColor(this.choiceRow[0].childNodes,false);
				TbBTGridEvent.setRowColor(curRow[0].childNodes,true);
			}
	...
	keyDown : function(Fields,aEvent){ //StringGrid 接收外部的keydown事件
		...
		case 40: //down
			if(this.choiceRow[0] == null ){ //如果当前没有选中行，则选中第一行
				...
				TbBTGridEvent.setRowColor(this.choiceRow[0].childNodes,true);
			}else{
				var inRow = TbBTGridEvent.isDisplay($(this.choiceRow[0]),"next");
				if(inRow != null && inRow.attr("Ix") != null){
					TbBTGridEvent.setRowColor(this.choiceRow[0].childNodes,false);
					TbBTGridEvent.setRowColor(inRow.children(),true);
					this.choiceRow[0] = inRow[0];
				}else
					return;
				TbBTGridEvent.setScrollTop(this,"down",inRow);
			}
			break;
		case 38: //up
			if(this.choiceRow[0] != null ){ //如果当前没有选中行，则选中第一行
				var inRow = TbBTGridEvent.isDisplay($(this.choiceRow[0]),"prev");
				if(inRow != null && inRow.attr("Ix") != null){
					TbBTGridEvent.setRowColor(this.choiceRow[0].childNodes,false);
					TbBTGridEvent.setRowColor(inRow.children(),true);
					this.choiceRow[0] = inRow[0];
					...
					return;
				TbBTGridEvent.setScrollTop(this,"up",inRow);
			}
			break;
		case 13 : //回车选中值
...
var TbBTGridEvent = {		
	...
	setColColor : function(gridThis, inCellObj, tag){ //单元格事件
		if(!gridThis.isSetColColor){
			return;
		}
		var name = inCellObj.getAttribute("name");
		var fieldname = inCellObj.getAttribute("fieldname");
		$("#"+gridThis.eleId).find("[fieldname='"+fieldname+"']").each(function(j,obj){
			if(obj.tagName == "TD")
				obj.style["background"] = tag?"rgb(220, 248, 168)":""; //#B8CFEE,#c4d5ee
		});
	},
	...
	bindCellAffair : function($inCell,gridThis,inTag,aEvent){ //绑定单元格事件
		if(inTag){
			if(gridThis["onCellClick"] != null ){ //系统使用的单击事件
				var evtents =  $inCell.data("events") || $._data($inCell[0],"events");
				if(evtents == null || evtents["click"] == null)
					$inCell.click(function(aEve){return gridThis["onCellClick"](gridThis.rowIndex(), $inCell[0], aEve)});
			}
		}else{
			if(gridThis["onCellClick"] != null){ 
				$inCell.unbind("click");
			}
		}
	}
...
var TbBTGridCell = Class.create();
TbBTGridCell.prototype = {
	cell     : null,	
